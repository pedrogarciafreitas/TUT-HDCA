...
void start_decoding( FILE* outputFile )
{   
	int i;
    value = 0;					/* Input bits to fill the   */
    for (i = 1; i<=Code_value_bits; i++) {	/* code value.              */
        value = 2*value+input_bit(outputFile);
    }
...
/*
int saveReg(FILE* fid)
{
	fid = fopen("positions.txt","a");
	if (fid == NULL)
		return 1;
	fprintf(fid, "%d %d %d", value, low, high );
	fclose(fid);
	return 0;
}
*/
...
/* LOAD THE "REGISTERS"*/
/*int loadReg(int val, int lo, int hi)
{
	value = val;
	low = lo;
	high = hi;
...
int decode_symbol(int cum_freq[],FILE* outputFile)
{   long range;			/* Size of current code region              */
    ...
    for (;;) {					/* Loop to get rid of bits. */
        ...
        //printf("decode_symbol\n");
        value = 2*value+input_bit(outputFile);		/* Move in next input bit.  */
    }
    return symbol;
}
